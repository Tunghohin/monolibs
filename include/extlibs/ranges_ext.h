// #ifndef _RANGES_EXT_H
// #define _RANGES_EXT_H

// #include <extlibs/concepts_ext.h>
// #include <ranges>

// namespace mono {

// template <template <typename...> typename C, std::ranges::input_range R,
//           typename... Args, typename ContainerType>
// requires mono::IsContainer<ContainerType>
// constexpr auto to(R&& r, Args&&... args) -> ContainerType {
//     return to<ContainerType>(std::forward<R>(r),
...
//     closure_range(Brgs&&... brgs) : args_(std::forward<Brgs>(brgs)...) {}
//     std::tuple<Args...> args_;
// };

// template <std::ranges::input_range R, std::ranges::input_range C,
//           typename... Args>
// auto constexpr operator|(R&& r, closure_range<C, Args...>&& c) {
//     return std::apply(
//         [&r](auto&&... inner_args) { return to<C>(std::forward<R>(r)); },
//         std::move(c.args_));
// }
...
// struct closure_ctad {
//     template <typename... Brgs>
//     closure_ctad(Brgs&&... brgs) : args_(std::forward<Brgs>(brgs)...) {}
//     std::tuple<Args...> args_;
// };
...
constexpr auto to(R&& r, Args&&... args) -> ContainerType {
    return tl::to<ContainerType>(std::forward<R>(r),
                                 std::forward<Args>(args)...);
}
...
auto constexpr operator|(R&& r, closure_ctad<C, Args...>&& c) {
    return std::apply(
        [&r](auto&&... inner_args) { return tl::to<C>(std::forward<R>(r)); },
        std::move(c.args_));
}
} // namespace detail
